...
inline void assignTriangleNormalArea(Triangle &tri, const std::vector<Vertex> &verts) {
    ...
    // Ordered relative to Gmsh indexing and coord system.
...
inline void assignVertTBNGmsh(Vertex &vert, const std::vector<uint32_t> &vertTriInds,
    ...
    spdlog::debug("Computing vertex normal:");
    ...
    for (uint32_t vertTriIdx : vertTriInds) {
        ...
        spdlog::debug(" - Averaging tri normal: {}", math_util::debugGlmVecTrunc(triDVec));
        normal += vertTri.area * triDVec;
    }
    normal = -1.0 * glm::normalize(normal);
    spdlog::debug(" :: Computed normal: {}", math_util::debugGlmVecTrunc(normal));
    ...
    spdlog::debug(" :: Computed tangent: {}", math_util::debugGlmVecTrunc(tangent));
    spdlog::debug(" :: Computed bitangent: {}", math_util::debugGlmVecTrunc(bitangent));
